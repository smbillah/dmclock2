...
//    } // TEST
	...
	if (!std::isnan(prop1)) {
      ...
      auto add_f = [&](ClientId& cl, int count = 1, Time t = dmc::get_time(),  bool should_print = true) {
	++pq->tick;
	bool is_new = false;
	...
	} else { // temporarily add new client before calling
	  is_new = true;
	...
	for (int i = 0 ; i < count ; i++) {
      ...
      auto pop_f = [&] (ClientId& cl, bool should_print = true) {
	...
	if (should_print) {
      ...
      auto just_pop_f = [&] (bool should_print = true) {
	Queue::PullReq pr = pq->pull_request();
	if(pr.is_retn()){
	  if (should_print) {
	    std::cout << "popped: " << pr.get_retn().client << std::endl;
	...
	std::cout <<"limit: "; print_f(pq->limit_heap.top().client);
      ...
      //add another client -- all clients are ready
      add_f(client5);
      print_all_f();
